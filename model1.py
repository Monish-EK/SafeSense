...
# Train face recognizer from folders (each folder is one person's identity)
def train_face_recognizer(face_folder_path):
    ...
    for person_name in os.listdir(face_folder_path):
        ...
        if os.path.isdir(person_path):  # Check if it's a folder
            ...
            label_map[current_label] = person_name  # Assign label to the person's name
            current_label += 1  # Increment label for the next person
...
def detect_safety_gear_and_faces():
    ...
    logged_names_today = set()  # To prevent duplicate emails
    ...
    while True:
        ...
        # If no safety gear, check face recognition
        if not safety_gear_detected:
            ...
            for (x, y, w, h) in faces:
                ...
                if name != "Unknown" and name not in logged_names_today:
                    ...
                    os.makedirs("screenshots", exist_ok=True)  # Create folder if not exists
                    cv2.imwrite(screenshot_path, frame)

                    log_violation(name, screenshot_path)
                    notify_hr(name, screenshot_path)
                    logged_names_today.add(name)  # Mark person as notified
